...
export class ZcashTransactionBuilder extends UtxoTransactionBuilder<
  ZcashTransaction
> {
  ...
  setDefaultsForVersion(network: ZcashNetwork, version: number): void {
    ...
    this.tx.consensusBranchId = getDefaultConsensusBranchIdForVersion(
      network,
      version,
    );
  }
  ...
  private hasSignatures(): boolean {
    return (this as any).__INPUTS.some(function(input: {
      signatures: unknown;
    }) {
  ...
  private setPropertyCheckSignatures(
    propName: keyof ZcashTransaction,
    value: unknown,
  ) {
