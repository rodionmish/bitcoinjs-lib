...
import { Transaction, TxInput, TxOutput } from '../..';
 ...
 */
export function getBlake2bHash(
  buffer: Buffer,
  personalization: string | Buffer,
): Buffer {
...
export function getPrevoutsDigest(
  ins: TxInput[],
...
export function getSequenceDigest(
  ins: TxInput[],
...
export function getOutputsDigest(
  outs: TxOutput[],
...
function getTransparentDigest(
  tx: { ins: TxInput[]; outs: TxOutput[] },
  sigParams?: SignatureParams,
): Buffer {
...
function getSaplingDigest(_tx: ZcashTransaction): Buffer {
  // https://zips.z.cash/zip-0244#t-3-sapling-digest
  return getBlake2bHash(Buffer.of(), 'ZTxIdSaplingHash');
}

function getOrchardDigest(_tx: ZcashTransaction): Buffer {
...
function getDigest(
  ...
  const personalization = BufferWriter.withCapacity(
    tag.length + 4 /* UInt32 */,
  );
