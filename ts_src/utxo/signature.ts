...
import { payments, script, Transaction, TxInput, TxOutput } from '..';
...
export function getDefaultSigHash(
  network: Network,
...
export function parseSignatureScript(
  ...
  if (isSegwitInput) {
    ...
    if (isNativeSegwitInput) {
      inputClassification = classify.witness(
        decompiledSigScript as Buffer[],
        true,
      ) as InputType;
    } else {
...
export function parseSignatureScript2Of3(
  input: TxInput,
): ParsedSignatureScript2Of3 | ParsedSignatureScriptTaproot {
 ...
 */
function isSignatureByPublicKey(
...
export function signInput2Of3(
  ...
  if (scriptType === 'p2tr') {
    ...
    ({ redeemScript, witnessScript } = createOutputScript2of3(
      pubkeys,
      scriptType,
    ));
  }
