...
import { Network, Payment, payments } from '..';
 ...
 */
export function isSupportedScriptType(_network: Network, scriptType: ScriptType): boolean {
  switch (scriptType) {
    case 'p2sh':
    case 'p2shP2pk':
      return true;
...
export function createOutputScriptP2shP2pk(pubkey: Buffer): SpendableScript {
  const p2pk = payments.p2pk({ pubkey });
  const p2sh = payments.p2sh({ redeem: p2pk });
...
export function createOutputScript2of3(
  ...
  const script2of3 = payments.p2ms({ m: 2, pubkeys });
  assert(script2of3.output);

  let scriptPubKey: Payment;
  let redeemScript: Payment | undefined;
  let witnessScript: Payment | undefined;
  switch (scriptType) {
    case 'p2sh':
      redeemScript = script2of3;
      scriptPubKey = payments.p2sh({ redeem: script2of3 });
      break;
    case 'p2shP2wsh':
      witnessScript = script2of3;
      redeemScript = payments.p2wsh({ redeem: script2of3 });
      scriptPubKey = payments.p2sh({ redeem: redeemScript });
      break;
    case 'p2wsh':
      witnessScript = script2of3;
      scriptPubKey = payments.p2wsh({ redeem: witnessScript });
      break;
    default:
